...
class BloxorzSuite {
  ...
  @Test def `neighbors history level 1 test`: Unit =
    new Level1 {
  ...
  @Test def `only neighbors history only level 1 test`: Unit =
    new Level1 {
      val expectedResult =  Set(
        (Block(Pos(2,1),Pos(3,1)), List(Down,Left,Up))
      ).to(LazyList)
  ...
  @Test def `optimal solution identity for level 1`: Unit =
    new Level1 {
      assertEquals(optsolution, solution)
    }

  @Test def `only legal blocks in solution`: Unit = {
    new Level1 {
      val sol = pathsToGoal filter { case (block, moves) => !block.isLegal}
      assert(sol.isEmpty)
    }
  }
