...
trait Solver extends GameDef {
  ...
  type BlockList = LazyList[(Block, List[Move])]

  def neighborsWithHistory(b: Block, history: List[Move]): BlockList = {
    (for {
   ...
   */
  def newNeighborsOnly(neighbors: BlockList,
                       explored: Set[Block]): BlockList = {
    (for {
      (block, move) <- neighbors
      if !(explored contains block)
    } yield (block, move)).to(LazyList)
  }
  ...
  def from(initial: BlockList,
           explored: Set[Block]): BlockList = initial.toList match {
    case Nil => LazyList()
    case (block, moves) :: xs => {
      val newExplored = explored + block
      val newNeighbors = newNeighborsOnly(neighborsWithHistory(block, moves), newExplored)
      newNeighbors ++ from(xs.to(LazyList) ++ newNeighbors, newExplored)
    }
  }
   ...
   */
  lazy val pathsFromStart: BlockList =
   ...
   */
  lazy val pathsToGoal: BlockList =
