...
trait EncoderInstances {
  ...
  // TODO Define an implicit value of type `Encoder[String]`
...
trait DecoderInstances {
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(items) => Option(items flatMap (item => item.decodeAs(decoder)))
  ...
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction {
      case Json.Obj(obj) => obj.get(name) match {
        case Some(x) => x.decodeAs(decoder)
...
trait ContactsCodecs {
  ...
  implicit def encode[Contacts](implicit personEncoder: Encoder[Person]): List[Person] => List[Json] =
    (people: List[Person]) => for {
      person <- people
    } yield personEncoder.encode(person)

  implicit def decode[Contacts](implicit personDecoder: Decoder[Person]): List[Json] => List[Option[Person]] =
    (people: List[Json]) => for {
      person <- people
    } yield personDecoder.decode(person)
